                          ...
                          const Vector3f& dir, float& tnear, float& u, float& v)
{
   ...
   // Also don't forget to update tnear, u and v.
    ...
    float fract=dotProduct(S1,E1);
    float reci=1/fract;

    // we need to check barycentric coordinate b1, b2, b3 are valid or not.
    u=dotProduct(S1,S)*reci;
    if (u<0 || u>1) return false;

    v=dotProduct(S2,dir)*reci;
    if (v<0 || v>1) return false;

    if (1-u-v<0 || 1-u-v>1) return false;

    tnear=dotProduct(S2,E2)*reci;
