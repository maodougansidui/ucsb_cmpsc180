...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
	...
	int x_l = std::floor(std::min(v[0][0], std::min(v[1][0], v[2][0])));   //floor rounded down
	int x_r = std::floor(std::max(v[0][0], std::max(v[1][0], v[2][0])));    //ceil rounded up
	int y_b = std::floor(std::min(v[0][1], std::min(v[1][1], v[2][1])));
	int y_t = std::floor(std::max(v[0][1], std::max(v[1][1], v[2][1])));

	for (int x = x_l; x <= x_r; x++){
        for (int y = y_b; y <= y_t; y++) {

            if (insideTriangle(x + 0.5, y + 0.5, t.v)) {
                auto[alpha, beta, gamma] = computeBarycentric2D((float)x + 0.5f, (float)y + 0.5f, t.v);  //Barycentric coordinate interpolation
